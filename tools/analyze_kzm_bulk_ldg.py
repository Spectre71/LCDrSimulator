...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Analyze rate-sweep outputs from the periodic bulk Landau-de Gennes KZM branch.",
    )
    ...
    parser.add_argument(
        ...
        help="Comma-separated avg_S milestones for measure-mode=avg_s.",
    )
    parser.add_argument(
        "--extra-after-target-s",
        type=float,
        default=0.0,
        help="Optional delay added after the avg_S milestone time.",
    )
    parser.add_argument(
        "--observables",
        type=str,
        default="defect_line_density,defect_density_per_plaquette,xi_grad_proxy",
...
def select_index(
    ...
    if measure_mode == "avg_s":
        mask = (time_s >= tc_cross_s - 1e-18) & np.isfinite(avg_s) & (avg_s >= avg_s_target)
        if not np.any(mask):
            raise ValueError(f"avg_S never reaches target {avg_s_target}")
...
def main() -> int:
    ...
    analysis_dir = args.analysis_dir.resolve() if args.analysis_dir else (sweep_root / "analysis_bulk_ldg").resolve()
    analysis_dir.mkdir(parents=True, exist_ok=True)

    observables = parse_csv_list(args.observables)
    targets = [float("nan")] if args.measure_mode in ("final", "fixed") else parse_float_list(args.avg_s_targets)
    ...
    for observable in observables:
        for target in targets:
            ...
            after_tc_values: list[float] = []
            selected_avg_s_values: list[float] = []
            ...
            for case in cases:
                ...
                avg_s_value = float(series(log, "avg_S")[idx])
                ...
                detail_rows.append(
                    {
                        ...
                        "target_avg_S": float(target) if args.measure_mode == "avg_s" else float("nan"),
                        "extra_after_target_s": float(args.extra_after_target_s) if args.measure_mode == "avg_s" else 0.0,
                        "rate_abs": rate_abs,
                        "tau_Q_s": tau_q_s,
                ...
                after_tc_values.append(after_tc_actual_s)
                selected_avg_s_values.append(avg_s_value)

            rate_arr = np.array(rate_values, dtype=float)
            tau_arr = np.array(tau_values, dtype=float)
            obs_arr = np.array(obs_values, dtype=float)
            after_tc_arr = np.array(after_tc_values, dtype=float)
            avg_s_arr = np.array(selected_avg_s_values, dtype=float)
            ...
            summary_rows.append(
                {
                    ...
                    "positive_case_count": int(np.count_nonzero(np.isfinite(obs_arr) & (obs_arr > 0.0))),
    ...
    summary_path = analysis_dir / "kzm_bulk_ldg_summary.csv"
    ...
    for row in summary_rows:
        ...
        elif row["measure_mode"] == "avg_s":
            target_desc = f"avg_S={float(row['target_avg_S']):.3f}"
        else:
            target_desc = "final"
        print(
            f"[{row['observable']}] {target_desc} | "
            f"slope_vs_tauQ={float(row['slope_vs_tauQ']):.6g} "
            f"slope_vs_rate={float(row['slope_vs_rate']):.6g} "
            f"positive_cases={int(row['positive_case_count'])} "
            f"ratio={float(row['observable_ratio_max_over_min']):.6g}"
        )
...
if __name__ == "__main__":
    try:
        ...
        print(f"[kzm-bulk-ldg] ERROR: {exc}", file=sys.stderr)
        raise